...
# --- 1. アプリ基本設定 (必ず一番上に1回だけ書く) ---
st.set_page_config(page_title="Cell Stock & Seeding Manager", layout="centered", page_icon="🔬")

# --- 2. カスタムCSS (視認性向上・全角文字を排除) ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;800&family=JetBrains+Mono:wght@500&display=swap');
    ...
    /* Input Box: White text and dark background */
    ...
    /* Instruction bars (High contrast) */
    .instruction-blue {
...
# --- 2. カスタムCSS（iPhone最適化・ダークモード視認性向上） ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;800&family=JetBrains+Mono:wght@500&display=swap');
    ...
    /* 【重要】入力ボックス：文字を白に戻し、背景を暗く固定 */
    div[data-baseweb="input"] {
        background-color: #161b22 !important; /* 深いグレー */
        border: 1px solid #30363d !important;
        border-radius: 8px !important;
    }
    
    input {
        color: #ffffff !important; /* 白文字に復帰 */
        -webkit-text-fill-color: #ffffff !important;
        font-family: 'JetBrains Mono', monospace !important;
